(() => {
  ...
  // PUBLIC PREVIEW FIXTURES ONLY.
  // Every identity, case number, amount and entity below is synthetic and intentionally fictitious.
  // Never copy production, client, employee or real case data into this file.
  const users = [
    { id: '10000000-0000-0000-0000-000000000001', name: 'Usuário Exemplo Admin', email: 'admin@example.invalid', role: 'admin', active: true, task_access_enabled: false, allowed_modules: [] },
    { id: '10000000-0000-0000-0000-000000000002', name: 'Usuária Exemplo Tarefas', email: 'tarefas@example.invalid', role: 'task_only', active: true, task_access_enabled: true, allowed_modules: ['acordos'], otp_requests_24h: 1, otp_limit: 2, otp_limit_blocked: false },
    { id: '10000000-0000-0000-0000-000000000003', name: 'Usuário Exemplo Worker', email: 'worker@example.invalid', role: 'task_worker', active: true, task_access_enabled: true, allowed_modules: ['acordos'], otp_requests_24h: 2, otp_limit: 2, otp_limit_blocked: true },
    { id: '10000000-0000-0000-0000-000000000004', name: 'Usuário Exemplo Leitura', email: 'leitura@example.invalid', role: 'user', active: true, task_access_enabled: false },
    { id: '10000000-0000-0000-0000-000000000005', name: 'Usuária Exemplo Inativa', email: 'inativa@example.invalid', role: 'task_worker', active: false, task_access_enabled: false },
  ];
  ...
  const permissionOverrides = Object.fromEntries(users.map((user) => [user.id, Object.fromEntries(permissionDefinitions.map((permission) => [permission.key,
    user.email === 'admin@example.invalid' || user.role === 'admin' || (['task_worker', 'task_only'].includes(user.role) && ['tasks.view', 'tasks.execute'].includes(permission.key))
  ]))]));
  ...
  const task = { id: '20000000-0000-0000-0000-000000000001', type: 'acordos', title: 'Acordos — Base Fictícia', description: 'Saneamento de uma base sintética para prévia local.', priority: 'high', status: 'in_progress', total_processes: 25, completed_processes: 4, updated_at: '2026-01-15T13:45:00-04:00' };
  task.participant_user_ids = [users[0].id, users[1].id];
  const otherTask = { id: '20000000-0000-0000-0000-000000000002', type: 'liminar', title: 'Analisar Pedido de Tutela — Mock', description: 'Análise fictícia para testar a interface.', priority: 'medium', status: 'pending', total_processes: 10, completed_processes: 2, updated_at: '2026-01-15T12:20:00-04:00' };
  const processes = [
    { id: '30000000-0000-0000-0000-000000000001', task_id: task.id, case_number: '0000000-00.0000.0.00.0001', status: 'in_progress', assignee_id: users[0].id, provision_amount: '1000.00' },
    { id: '30000000-0000-0000-0000-000000000002', task_id: task.id, case_number: '0000000-00.0000.0.00.0002', status: 'pending', assignee_id: users[0].id, provision_amount: '2000.00' },
    { id: '30000000-0000-0000-0000-000000000003', task_id: task.id, case_number: '0000000-00.0000.0.00.0003', status: 'pending', assignee_id: users[0].id, provision_amount: '3000.00' },
  ];
  const agreements = [
    ['0000000-00.0000.0.00.0001','1000.00','completed','eligible','Apto para acordo.','revisao_juros','revisional','800.00','1000.00','2026-01-21',5,30,'pre_agreement','2026-01-15T13:42:00-04:00','Usuário Exemplo Admin'],
    ['0000000-00.0000.0.00.0004','2000.00','completed','ineligible','Possui sentença.',null,null,null,null,null,null,null,null,'2026-01-15T13:31:00-04:00','Usuária Exemplo Tarefas'],
    ['0000000-00.0000.0.00.0003','3000.00','pending',null,null,null,null,null,null,null,null,null,null,null,null],
    ['0000000-00.0000.0.00.0005','4000.00','completed','eligible','Apto para acordo.','cobranca_indevida',null,'3500.00','4000.00','2026-01-20',5,30,'pre_agreement','2026-01-14T16:08:00-04:00','Usuário Exemplo Worker'],
    ['0000000-00.0000.0.00.0006','5000.00','completed','ineligible','Possui Termo de Impedimento 12.',null,null,null,null,null,null,null,null,'2026-01-14T15:21:00-04:00','Usuário Exemplo Leitura'],
    ['0000000-00.0000.0.00.0007','6000.00','in_progress',null,null,null,null,null,null,null,null,null,null,null,'Usuário Exemplo Admin'],
    ['0000000-00.0000.0.00.0008','7000.00','completed','eligible','Apto para acordo.','outros','exibicao','6500.00','7000.00','2026-01-19',5,30,'pre_agreement','2026-01-13T11:14:00-04:00','Usuária Exemplo Tarefas'],
    ['0000000-00.0000.0.00.0009','8000.00','pending',null,null,null,null,null,null,null,null,null,null,null,null],
  ].map((values, index) => ({
  ...
  async function handle(path, options = {}) {
    ...
    if (url.pathname === '/api/pagamentos/resumo') return { cards: { total_pago: 9000, quantidade_pagamentos: 3, ticket_medio: 3000, tempo_medio_pagamento_dias: 5, credenciados_ativos: 1 } };
    if (url.pathname === '/api/pagamentos') return { rows: [{ pasta: 'MOCK-0001', credenciado: 'Credenciado Fictício', solicitante: 'Solicitante Exemplo', tipo_pagamento: 'Acordo', valor: 3000, situacao: 'Pago', data_aprovacao: '2026-01-02', data_pagamento: '2026-01-07', tempo_pagamento_dias: 5 }], page: 1, page_size: 25, total: 1, total_pages: 1 };
    if (url.pathname === '/api/pagamentos/credenciados') return { rows: ['Credenciado Fictício'] };
