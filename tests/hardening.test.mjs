...
const [auth, api, dashboard, config] = await Promise.all([
...
assert.match(config, /mbaIsLocal[\s\S]*\? 'http:\/\/localhost:5000'[\s\S]*mba-backoffice-proxy-production/, 'localhost must call the local Flask backend instead of Railway');
