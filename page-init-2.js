...
(() => {
  ...
  const baseTypes = {
    base_benner: { label: 'Upload - Base do Benner', endpoint: '/api/base-processual/benner/importar', successName: 'Base do Benner' },
    base_cpj: { label: 'Upload - Base do CPJ', endpoint: '/api/base-processual/cpj/importar', successName: 'Base do CPJ' },
  };
  ...
  const canImportBases = () => currentUser?.permissions?.['bases.import'] === true;

  function removeLegacyRows() { document.querySelectorAll('[data-base-upload-task]').forEach((row) => row.remove()); }
  ...
  function syncOptions() {
    ...
    Object.entries(baseTypes).forEach(([value, config]) => {
      ...
      if (allowed && !option) {
        option = document.createElement('option'); option.value = value; option.textContent = config.label; typeSelect.appendChild(option);
      } else if (!allowed && option) {
        if (typeSelect.value === value) typeSelect.value = 'liminar'; option.remove();
      }
    });
  }

  function syncMode() {
    const type = currentType(); const baseMode = isBaseType(type);
    ...
    if (agreementHelp) {
      ...
      agreementHelp.textContent = baseMode ? 'Envie a planilha XLSX bruta da execução mais recente.' : defaultAgreementHelp;
    }
    if (headerCopy) headerCopy.textContent = baseMode ? `Importe a ${baseTypes[type].successName.toLowerCase()} para atualizar a base processual.` : defaultHeaderCopy;
    if (submit) submit.textContent = baseMode ? 'Importar base' : 'Criar e importar';
    if (errorBox) errorBox.hidden = true;
  }

  async function submitBase(event) {
    const type = currentType(); if (!isBaseType(type)) return;
    event.preventDefault(); event.stopImmediatePropagation();
    const config = baseTypes[type]; const file = fileInput.files?.[0];
    if (!canImportBases()) { errorBox.textContent = 'Você não possui permissão para importar bases.'; errorBox.hidden = false; return; }
    if (!(file instanceof File) || !/\.xlsx$/i.test(file.name)) { errorBox.textContent = 'Selecione uma planilha XLSX.'; errorBox.hidden = false; return; }
    const originalLabel = submit.textContent; submit.disabled = true; submit.textContent = 'Importando…'; errorBox.hidden = true;
    const body = new FormData(); body.append('file', file);
    try {
      const result = await window.MBA_API.request(config.endpoint, { method: 'POST', body });
      const imported = result?.importacao?.valid_rows; const total = result?.importacao?.total_rows; const rejected = result?.importacao?.rejected_rows;
      form.reset(); syncMode(); taskDialog.close();
      const summary = Number.isFinite(Number(imported)) ? `${imported} linha(s) válida(s)${Number.isFinite(Number(total)) ? ` de ${total}` : ''}${Number(rejected) ? `; ${rejected} rejeitada(s)` : ''}.` : 'Importação concluída.';
      window.alert(`${config.successName} importada com sucesso. ${summary}`);
    } catch (error) {
      const messages = { 401: 'Sua sessão expirou. Entre novamente.', 403: 'Você não possui permissão para importar bases.', 409: 'Este arquivo já foi importado.', 413: 'O arquivo excede o limite permitido.' };
      errorBox.textContent = messages[error.status] || error.message || 'Não foi possível importar a base.'; errorBox.hidden = false;
  ...
  document.getElementById('new-task-button')?.addEventListener('click', () => { syncOptions(); syncMode(); });
  taskDialog.addEventListener('close', () => { if (!isBaseType()) syncMode(); });

  window.addEventListener('mba:authenticated', (event) => {
    currentUser = event.detail || null;
    window.MBA_CURRENT_USER = currentUser;
    removeLegacyRows(); syncOptions(); syncMode();
  });
  window.addEventListener('mba:session-expired', () => {
    currentUser = null; window.MBA_CURRENT_USER = null; removeLegacyRows(); syncOptions();
  });
