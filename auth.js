(() => {
  ...
  document.getElementById('task-worker-login-form')?.addEventListener('submit',async(event)=>{event.preventDefault();clearError();const button=document.getElementById('task-worker-login-submit');button.disabled=true;try{const session=await automationApi.request('/api/task-worker/session',{method:'POST',body:JSON.stringify({email:event.currentTarget.elements.email.value.trim()})});window.sessionStorage.setItem('mba_task_worker_token',session.access_token);window.sessionStorage.removeItem('mba_task_otp_request');applyUser(await automationApi.request('/api/me'));setAuthState(true);}catch(error){window.sessionStorage.removeItem('mba_task_worker_token');showError(error.message);}finally{button.disabled=false;}});
  document.getElementById('google-login')?.addEventListener('click',async()=>{clearError();if(!supabase)return showError(window.MBA_SUPABASE_ERROR||'Supabase não configurado.');const {error}=await supabase.auth.signInWithOAuth({provider:'google',options:{redirectTo:`${location.origin}${location.pathname}`}});if(error)showError(error.message);});
  document.getElementById('logout-button')?.addEventListener('click',async()=>{if(window.MBA_LOCAL_PREVIEW)return;if(window.sessionStorage.getItem('mba_task_worker_token')){await automationApi.request('/api/task-worker/session',{method:'DELETE'}).catch(()=>{});window.sessionStorage.removeItem('mba_task_worker_token');location.assign('/');return;}authGeneration+=1;await supabase?.auth.signOut();location.assign('/');});
  supabase?.auth.onAuthStateChange((event,session)=>{if(event==='SIGNED_IN'||event==='TOKEN_REFRESHED')setTimeout(()=>loadAdministrativeProfile(session).catch((error)=>{setAuthState(false);showError(error.message);}),0);if(event==='SIGNED_OUT'){authGeneration+=1;setAuthState(false);}});
  initializeAuth();
})();
